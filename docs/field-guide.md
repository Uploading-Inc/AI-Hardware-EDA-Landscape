...
| Repository-scale RTL | Partial design or issue plus repository context | Completion, reasoning answer, or repository patch | Single-shot model or long-horizon coding agent | Textual/formal scoring or native project verification | More realistic context, but oracle strength varies; leakage and maintenance are difficult |
| HLS and co-design | Specification or existing C/C++ and system context | Synthesizable accelerator and sometimes integration code | Single-shot HLS baselines; tool-using co-design agents | Simulation, synthesis, or deployment | Executable HLS stages exist; full system integration is less established |
